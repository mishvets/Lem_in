...
int			ft_valid_room(char **tmp, t_room_lst *r_lst, char *line, t_general *farm)
{
...
int			ft_room_read(char *line, t_room_lst **r_lst, t_general *farm)
{
	char		**tmp;
	t_room_lst	*new;
	...
	if (ft_valid_room(tmp, *r_lst, line, farm))
	{
		ft_strdeli(tmp, 3);
		return (1);
	}
